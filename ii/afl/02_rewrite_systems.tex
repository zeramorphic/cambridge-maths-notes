...
\begin{proof}
	...
	Similarly, if we have a \( G' \)-derivation that contains no letters anywhere, all strings occurring are of the form \( X(\alpha) \) for some \( \alpha \in \Omega^\star \), and the operation of replacing all occurences of \( X_a \) with \( a \) transforms that derivation into a \( G \)-derivation.
