...
\begin{proof}
	...
	\begin{align*}
		...
		&= \qty[1 - \frac{u^2}{2n} + o(\frac{1}{n})]^n
	\end{align*}
	The complex logarithm satisfies \( \log(1 + z) = z + o(z) \), so by taking logarithms, we find
	\[ \log \varphi_n(u) = n \log\qty(1 - \frac{u^2}{2n} + o(\frac{1}{n})) = -\frac{u^2}{2} \]
...
\begin{proof}
	...
	So we can compute
	\[ \expect{\qty(\sum{i=1}^n X_i)^4} = \expect{\sum_{i=1}^n X_i^4} + 6\expect{\sum_{i \neq j} X_i^2 X_j^2} \leq nM + 3n(n-1)M \leq 3n^2 M \]
...
% TODO: pull this and previous subsection into a new file
...
\begin{theorem}[Birkhoff]
	...
	Then \( \frac{S_n(f)}{n} \to \overline f \) almost everywhere, where \( \overline f \in L^1(\mu) \) is \( \Theta \)-invariant.
\end{theorem}
