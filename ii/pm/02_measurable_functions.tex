...
\begin{definition}
	...
	This collection of random variables is called \emph{independent} if the \( \sigma \)-algebras \( \sigma(X_i^{-1}(A) \colon A \in \mathcal E) \) are independent.
\end{definition}
For \( (E, \mathcal E) = (\mathbb R, \mathcal B) \) we show on the example sheet that this is equivalent \( I \) to the condition
...
Hence \( \mu_{Y_n} = \eval \mu_{(0,1)} \) by the uniqueness theorem, and so we have constructed an infinite sequence of independent uniform random variables \( Y_n \).
...
\begin{theorem}
	...
	If \( Y \colon (\mathbb R,\tau) \to (\mathbb R,\mathcal B) \) is measurable, it is constant almost surely.
\end{theorem}
\begin{proof}
