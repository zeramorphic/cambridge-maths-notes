...
\begin{example}
	...
	We can write a triangulation \( K \) of \( T^2 \) as \( K_1 \cup K_2 \).
	% TODO: diagram
	Then, \( \abs{K_i} \simeq S^1 \times I \), and \( \abs{K_1 \cap K_2} \simeq S_1^1 \amalg S_2^1 \).
	...
	% TODO: diagrams
	We have \( \ker \alpha_i \simeq \mathbb Z \) and \( \coker \alpha_i \simeq \mathbb Z \).
	...
	\end{cases} \]
\end{example}
It can be useful to note that if
% TODO: diagram
is exact, then \( B \simeq A \oplus \mathbb Z^r \).
\begin{example}
...
\begin{proof}
	...
	Note that since the first map in the last row is injective, the map before it is the zero map by exactness, so we can remove the bottom row and replace it with zero.
	We have that \( \partial_2 \) is injective, and \( H_1(L_{g,1}) \) is torsion-free, so \( \partial_2 \) is an isomorphism.
	Hence \( \iota \) is the zero map and \( j \) is an isomorphism.
	Since \( 0 = \iota = \iota_{g\star} + \iota'_\star \), we have \( \iota_{g\star} = 0 \).
