...
\begin{definition}
	An irreducible polynomial \( f \in K[T] \) is \emph{soluble by radicals} over \( K \) if there exists a sequence of fields \( K = K_0 \subseteq K_1 \subseteq \dots \subseteq K_m \), with \( x \in K_m \) a root of \( f \), and each \( K_{i+1} \) is obtained from \( K_i \) by adjoining a root, so \( K_{i+1} = K_i(y_i) \) where \( y_i^{d_i} \in K_i \).
\end{definition}
\begin{remark}
