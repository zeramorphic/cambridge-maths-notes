...
\begin{proof}
    ...
    Let
    \[ S = \qty{(y_1, \dots, y_r, z_r, \dots, z_s) \in \mathbb R^r \times \mathbb C^2 \simeq \mathbb R^n \midd \abs{y_i} \leq c_i, \abs{z_j}^2 \leq c_{r+j}} \]
...
Let \( u_k = \alpha_t \alpha_{t'}^{-1} \); this is a unit in \( \mathcal O_L \) such that \( \ell(u_k) = \ell(a_t) - \ell(a_{t'}) = (y_1, \dots, y_{r+s}) \) has \( y_i < 0 \) if \( i \neq k \).
