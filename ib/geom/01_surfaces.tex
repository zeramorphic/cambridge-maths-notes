...
\begin{example}
	...
	\end{lemma}
	\begin{proof}
		% TODO: add commutativity diagrams
...
\begin{example}
	...
	For instance, the first shape could also be written \( a b a^{-1} b^{-1} \).
	...
	\begin{lemma}
		\( \frac{P}{\sim} \), with the quotient topology, is a topological surface.
	\end{lemma}
	\begin{example}
		Consider the torus, defined here as \( [0,1]^2/\sim \).
		...
		Recall the gluing theorem from Analysis and Topology: that if \( X = A \cup B \) is a union of closed subspaces, and \( f \colon A \to Y \), \( g \colon B \to Y \) are continuous and \( \eval{f}_{A \cap B} = \eval{g}_{A \cap B} \), they define a continuous map on \( X \).
	\end{example}
\end{example}
