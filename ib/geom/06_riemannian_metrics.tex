...
\begin{definition}
	Let \( \Gamma \subseteq \mathbb C \) be a circle or line.
	We say that points \( z, z' \in \mathbb C \) are \textit{inverse for \( \Gamma \)} if every circle through \( z \) orthogonal to \( \Gamma \) also passes through \( z' \).
\end{definition}
\begin{lemma}
