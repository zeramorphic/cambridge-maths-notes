...
\begin{proof}
    ...
    The left hand side is not dependent on \(\vb z\), so by minimising \(\vb z\) we get the best bound for the left hand side. We set the gradient of \(\vb z\) to zero.
    \begin{align*}
...
for all \(\vb x\).
\begin{claim}
    Let \(f\) be \(\alpha\)-strongly convex. Let \(p^\star\) be the optimal cost; i.e.\ the minimum value of \(f\).
...
\begin{proof}
    ...
    \begin{align*}
        f(\vb x^\star) &\geq f(\vb x) + \grad{f(\vb x)}(\vb x^\star - \vb x) + \frac{\alpha}{2}\norm{\vb x^\star - \vb x}^2 \\
