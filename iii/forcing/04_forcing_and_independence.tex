...
\end{proof}
We can force inaccessible cardinals \( \lambda \) to become successor cardinals.
...
\end{remark}
\begin{corollary}
    If \( \mathsf{ZFC} + \mathsf{IC} \) is consistent, then so is \( \mathsf{ZFC} + \text{(\( \aleph_1^{\mathrm{V}} \) is inaccessible in \( \mathrm{L} \))} \).
\end{corollary}
\begin{proof}
