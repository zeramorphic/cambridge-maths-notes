...
\begin{proof}
    ...
    \[ C_n = \qty{A \in C \midd \mu(A) > \frac{1}{n}} \]
...
\begin{lemma}[Ulam]
    ...
    \begin{enumerate}
        \item for a given \( \xi \), the set \( \qty{A_\alpha^\xi \midd \alpha < \lambda^+} \) is a pairwise disjoint family; and
        \item for a given \( \alpha \), we have
...
However, we can emulate completeness for nonprincipal ultrafilters at the cardinal \( \kappa^+ \) using the following method.
...
\[ M \vDash \varphi\qty[\frac{a(0)a(1) \dots a(\xi)\dots}{v_0 v_1 \dots v_\xi \dots}] \]
\begin{definition}
...
\end{theorem}
\begin{proof}
    We define a language \( L \) extending the usual language of set theory by creating a constant symbol \( c_A \) for each \( A \subseteq \kappa \), giving \( 2^\kappa \)-many symbols.
