...
We wish to create a similar structure that is of the form \( V_\alpha \) for some \( \alpha \).
We define
\[ \alpha_0 = 0;\quad \alpha_{k+1} = \sup{\rank(x) \mid x \in W(V_{\alpha_k})};\quad \alpha = \sup{\alpha_n \mid n \in \mathbb N} \]
Note that \( N \subseteq V_{\alpha_1} \).
\begin{theorem}
    \( V_\alpha \preceq V_\kappa \) and \( \alpha < \kappa \).
\end{theorem}
\begin{proof}
    ...
    Now, if \( \alpha_k < \kappa \), we have \( \abs{V_{\alpha_k}} < \kappa \) by a previous lemma.
    Thus,
    \[ \abs{W(V_{\alpha_k})} \leq \aleph_0 \cdot \abs{V_{\alpha_k}^{<\omega}} = \abs{V_{\alpha_k}} < \kappa \]
    where \( X^{<\omega} \) is the set of finite sequences of elements of \( X \).
    Hence \( \qty{\rank(x) \mid x \in W(V_{\alpha_k})} \) is a set of less than \( \kappa \) ordinals less than \( \kappa \), so it must be bounded by regularity.
...
\begin{remark}
    ...
    In particular, \( V_\alpha \vDash \mathsf{ZFC} \) but \( \alpha \) is not inaccessible.
\end{remark}
\begin{definition}
    We call an ordinal \( \alpha \) \emph{worldly} if \( V_\alpha \vDash \mathsf{ZFC} \), and write \( \mathsf{Wor}(\alpha) \).
\end{definition}
...
\begin{proof}
    ...
    Assuming Kuratowski's definition of ordered pairs, an element of \( \lambda \times \lambda \) is an element of \( V_{\lambda} \), so \( \lambda \times \lambda \in V_{\lambda + 1} \) and \( R \in V_{\lambda + 1} \).
    The pair \( (\lambda, R) \) is an element of \( V_{\lambda + 3} \subseteq V_{\kappa} \).
    Thus \( V_\kappa \) contains a well-order \( (\lambda, R) \) of order type \( \kappa \).
    But \( \mathsf{ZFC} \) proves that every well-ordering is isomorphic to a unique ordinal, so we must have \( \kappa \in V_\kappa \), which is a contradiction.
\end{proof}
...
If \( \kappa \) is the least inaccessible cardinal, then \( V_\kappa \) is a model of \( \mathsf{ZFC} \), but we can show that it cannot satisfy \( \mathsf{IC} \).
Note that the statement `\( \lambda \) is inaccessible' is a \( \Pi_1 \) statement, so is downwards absolute.
Given a model with two inaccessible cardinals \( \kappa_0 < \kappa_1 \), we have \( V_{\kappa_1} \vDash \mathsf{ZFC} + \mathsf{I}(\kappa_0) \) so in particular, \( V_{\kappa_1} \vDash \mathsf{ZFC} + \mathsf{IC} \).
\begin{lemma}
    The formula `\( \lambda \) is inaccessible' is absolute for \( V_\kappa \) and \( V \).
\end{lemma}
In particular, \( V_\kappa \) above does not satisfy \( \mathsf{IC} \).
