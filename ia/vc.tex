...
Recall that $\vb F$ is conservative if we can write $\vb F = \grad f$. We say that $\vb F$ is irrotational if $\curl{\vb F} = \vb 0$. Hence, any conservative function is irrotational. The converse is true if the domain of $\vb F$ is 1-connected. We say that $\vb F$ is solenoidal if $\div{F} = 0$. If there exists a vector potential $\vb A$ for $\vb F$, i.e. $\vb F = \curl{\vb A}$, then $\vb F$ is solenoidal. The converse is true if the domain of $\vb F$ is 2-connected.
...
\begin{proof}
    ...
    \end{align*}
    Therefore,
    \[ \iint_{A} \left( \pdv{Q}{u} - \pdv{P}{v} \right) \dd{u}\dd{v} = \iint_{A} \left( \curl{\vb F} \right) \cdot \left( \pdv{\vb x}{u} \times \pdv{\vb x}{v} \right) = \iint_{S} (\curl{\vb F}) \dd{\vb S} \]
